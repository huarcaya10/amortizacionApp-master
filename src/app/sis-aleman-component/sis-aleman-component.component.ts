import { ThisReceiver } from '@angular/compiler';
import { Component, OnInit } from '@angular/core';
import { Periodo } from '../interface';

@Component({
  selector: 'app-sis-aleman-component',
  templateUrl: './sis-aleman-component.component.html',
  styleUrls: ['./sis-aleman-component.component.css']
})
export class SisAlemanComponentComponent implements OnInit {

  constructor() { }

  ngOnInit(): void {
  }
  public datos : Periodo [] = [];
  public dato : Periodo = this.datoEmpty();
  cantidad: any;
  interes: any;
  periodo: any ;

datoEmpty(): Periodo {
  return{
    periodo: 0,
    mensualidad: 0,
    interes: 0,
    amortizacion: 0,
    saldo: this.cantidad
  };
}
limpiar() : void {
  this.datos.splice(0,this.datos.length);
  this.cantidad=0
  this.interes=0
  this.periodo=0
}
onDatos(): void{
  const p: number = this.cantidad
  const n: number = this.periodo
  const i: number = this.interes/100
  const Aconstante : number = p/n
  this.dato = this.datoEmpty()
  this.datos.push(this.dato)
  for(let x =1;x<=n;x++){
    var inuevo : number = this.datos[x-1].saldo*i
    var saldo : number = Number((this.datos[x-1].saldo-Aconstante))
    var Mrenta: number = inuevo+Aconstante
    var datoo :Periodo = {
    periodo: x,
    mensualidad: Mrenta,
    interes: Number(inuevo.toFixed(5)),
    amortizacion: Aconstante,
    saldo: Number(saldo.toFixed(4))
    }
    this.datos.push(datoo);
  }
  console.log(this.dato)
}
}
